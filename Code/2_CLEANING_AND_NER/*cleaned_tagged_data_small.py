...
# Load dataset -> change later
mlx = load_dataset("isebire/multi_lexsum_TINY")
...
for split in ['validation']:   # ['train', 'validation', 'test']:
    ...
    #for case in [mlx[split][5]]:    <- for testing
        print('** Now analysing case ' + str(i) + ' in split')
    ...
    filename = 'mlx_tiny_clean_' + split + '.csv'
...
mlx_clean.push_to_hub("isebire/multi_lexsum_TINY_CLEANED", private=True)
