...
def ner_legalbert_passage_by_sentence(text, verbose=False):
    ...
    for sentence in sentences:
        ...
        for entity, type in all_entities_in_sentence:
            if entity not in entities_in_chain:
                chain_surface = chain_surface + entity + ' | '
                chain_label = chain_label + type + ' | '
                chain_both = chain_both + entity + ' ' + type + ' | '
                entities_in_chain.append(entity)
                entities_added = True
        if entities_added:
            chain_surface = chain_surface + ' ||| '
            chain_label = chain_label + ' ||| '
            chain_both = chain_both + ' ||| '

    chain_surface = chain_surface + '*'
    chain_surface = chain_surface.replace(' |  ||| ', ' ||| ')
    chain_surface = chain_surface.replace(' ||| *', '')
    ...
    return entities_w_labels, entities, labels, chain_surface, chain_label, chain_both
