...
class Rmgarbage(object):
    ...
    def bad_consonant_vowel_ratio(self, string):
        ...
        consonant_count = len(list(alpha_string)) - vowel_count
    ...
    def has_four_distinct_puncts_inside(self, string):
        """
        EDITED FROM 2 TO 4
        ...
        non_alnum_string = ''.join(char for char in string[1:-2]
                                   if not char.isalnum()).replace(' ','')
        if len(set(list(non_alnum_string))) >= 4:
            return True
        else:
            return False
    ...
    def same_punct_repeats(self, string):
        ''' custom added rule '''
        repeats = [match[0] for match in re.findall(r'((.)\2{1,})', string)]
        punct_repeats = []
        for repeat in repeats:
            # note - some puncts are acceptable to repeat
            if repeat[0] in [',', ':', ';', '~', '%', '$', '£']:
                punct_repeats.append(repeat)
        if len(punct_repeats) >= 1:
            return True
        return False

    def punct_between_lower(self, string):
        ''' custom added rule '''
    ...
    def is_garbage(self, string):
        ...
        if self.bad_alnum_ratio(string):
            return 'A'
        elif self.bad_consonant_vowel_ratio(string):
            return 'V'
        elif self.same_punct_repeats(string):
            return 'PR'
        elif self.punct_between_lower(string):
            return 'PL'
        return False
