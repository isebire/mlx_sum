...
def build_beam_json_for_bert(src_json_fp, dump_json_fp, beam, summary_size):
    ...
    print(f'start processing. Time: {time.process_time()}')
    with open(src_json_fp) as f, open(dump_json_fp, 'a') as dump_f:
        for line in f:
            ...
            print('just before the problem')
            beam2score = beam_selection(   # this is the problem!!!
            ...
            n_line += 1
            if n_line % 1 == 0:
                print(f'processed {n_line} lines. Time: {time.process_time()}')
