...
MODEL_PATH = f"{args.input_dir}/custom-legalbert"
...
lr = 2e-5
max_grad_norm = 1.0
num_training_steps = 12861   # changed from 1000!! (epochs * train egs ) / batch size = (3 * 68592) / 16
num_warmup_steps = 0.01 * num_training_steps    # changed from 100
warmup_proportion = float(num_warmup_steps) / float(num_training_steps)  # originally 0.1
...
for epoch in trange(epochs, desc="Epoch"):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        loss, logits = outputs[:2]    # original -> this loss is effectively discarded
        # train_loss_set.append(loss.item())

        # Custom loss -> removed while downsampling
        # logits = outputs.get('logits')
        # loss_fct = nn.CrossEntropyLoss(weight=torch.tensor([1, 200]))
        # loss = loss_fct(logits, b_labels)       #logits.view(-1, 2), b_labels.view(-1))

        # Now this is the loss used in backprop
        loss.backward()
...
prediction_labels = torch.tensor(labels).to(device)
#prediction_dataTypeId = torch.tensor(dataTypeId).to(device)
#prediction_fileId = torch.tensor(fileId).to(device)
#prediction_context_id = torch.tensor(context_id).to(device)
...
for batch in prediction_dataloader:
    ...
    batch = tuple(t.to(device) for t in batch)
    # Unpack the inputs from our dataloader
    b_input_ids, b_input_mask, b_labels = batch # , b_dataTypeId, b_fileId, b_context_id = batch
    ...
    eval_accuracy += tmp_eval_accuracy
    nb_eval_steps += 1
    confidence_flat = [x[1] for x in probabilities]   # check
    pred_flat = np.argmax(logits, axis=1).flatten()
    labels_flat = label_ids.flatten()
    count = 0
    for i in range(pred_flat.shape[0]):
        # iterate over the batch
        csv_output.append((b_input_ids[i], confidence_flat[i], pred_flat[i], labels_flat[i])) #b_dataTypeId[i], b_fileId[i], b_context_id[i]))
...
headings = ['context', '+ve confidence', 'predicted', 'label'] # 'dataTypeId', 'fileId', 'context_id']
df = pd.DataFrame(columns=headings)
for ids, conf, pred, label in csv_output: # dataTypeId, fileId, context_id in csv_output:
    ...
    data = [{'context': ' '.join(sentence), '+ve confidence': str(conf), 'predicted': str(pred), 'label': str(label)}] #, 'dataTypeId': str(dataTypeId),
             #'fileId': str(fileId), 'context_id': str(context_id)}]
    df = pd.concat([df, pd.DataFrame(data, columns=headings)])
