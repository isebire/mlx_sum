...
if START_FROM == 0:
    ...
    for extractive_output_col in extract_cols:
        results[extractive_output_col] = {'rouge_1_f1s': [], 'rouge_2_f1s': [], 'rouge_L_f1s': [], 'R_means': [], 'bs_f1s': []}
...
for index, case in enumerate(mlx_test):
    ...
    for extractive_output_col in extract_cols:
        ...
        results[extractive_output_col]['bs_f1s'].append(stats_results['bs_f1'])
        if verbose:
            print(stats_results)
...
metric_lists = ['rouge_1_f1s', 'rouge_2_f1s', 'rouge_L_f1s', 'bs_f1s', 'R_means']
metric_titles = {'rouge_1_f1s': 'ROUGE-1 F1', 'rouge_2_f1s': 'ROUGE-2 F1', 'rouge_L_f1s': 'Rouge-L F1', 'bs_f1s': 'BERTScore F1 (deberta-large-mnli)', 'R_means': 'Mean of ROUGE-1 F1 and ROUGE-2 F1'}
