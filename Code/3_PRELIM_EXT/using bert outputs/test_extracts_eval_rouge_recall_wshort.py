...
mlx_test = load_from_disk('mlx_pegasus_short_bert_sents_test.hf')
...
extract_cols = ['first_1024',
                'first_k',
                'bert_sents', 'bert_sents_short', 'oreo_sents',
...
for extractive_output_col in extract_cols:
    results[extractive_output_col] = {'rouge_1_recalls': [], 'rouge_2_recalls': [], 'rouge_L_recalls': [], 'R_means_recalls': []}
...
for index, case in enumerate(mlx_test):
    ...
    for extractive_output_col in extract_cols:
        ...
        results[extractive_output_col]['rouge_1_recalls'].append(stats_results['r1_recall'])
        results[extractive_output_col]['rouge_2_recalls'].append(stats_results['r2_recall'])
        results[extractive_output_col]['rouge_L_recalls'].append(stats_results['rL_recall'])
        if verbose:
            print(stats_results)

        R_mean = (stats_results['r1_recall'] + stats_results['r2_recall']) / 2
        results[extractive_output_col]['R_means_recalls'].append(R_mean)
        if verbose:
            print(R_mean)
    ...
    if results['bert_sents']['R_means_recalls'][-1] > results['oreo_sents']['R_means_recalls'][-1]:
        if verbose:
            print('bert better')
    ...
    if results['bert_windows']['R_means_recalls'][-1] > results['oreo_windows']['R_means_recalls'][-1]:
        if verbose:
            print('bert better')
    ...
    if results['bert_paras']['R_means_recalls'][-1] > results['oreo_paras']['R_means_recalls'][-1]:
        if verbose:
            print('bert better')
...
metric_lists = ['rouge_1_recalls', 'rouge_2_recalls', 'rouge_L_recalls', 'R_means_recalls']
metric_titles = {'rouge_1_recalls': 'ROUGE-1 Recall', 'rouge_2_recalls': 'ROUGE-2 Recall', 'rouge_L_recalls': 'Rouge-L Recall', 'R_means_recalls': 'Mean of ROUGE-1 Recall and ROUGE-2 Recall'}
for metric in metric_lists:
    for extractive_strat in extract_cols:
        ...
        fn = '2_' + metric + '_' + extractive_strat + '_dist'
    ...
    data = [results['first_1024'][metric], results['first_k'][metric], results['bert_sents'][metric], results['bert_sents_short'][metric], results['bert_windows'][metric], results['bert_paras'][metric], results['textrank_30'][metric]]
    fn = '2_6_box_' + metric
    graphs.box_plot(data, ['First 1024', 'First K', 'BERT - Sentences', 'BERT - Sentences (Short)', 'BERT - Windows', 'BERT - Paragraphs', 'TextRank'], metric_titles[metric], 'Extractive Strategy', metric_titles[metric], fn, long_labels=True)

    # Box plot for BERT vs OREO
    data = [results['bert_sents'][metric], results['bert_sents_short'][metric], results['oreo_sents'][metric], results['bert_windows'][metric], results['oreo_windows'][metric], results['bert_paras'][metric], results['oreo_paras'][metric]]
    fn = '2_oreo_bert_pairs_box_' + metric
    graphs.box_plot(data, ['BERT - Sentences', 'BERT - Sentences (Short)', 'OREO - Sentences', 'BERT - Windows', 'OREO - Windows', 'BERT - Paragraphs', 'OREO - Paragraphs'], metric_titles[metric], 'Extractive Strategy', metric_titles[metric], fn, positions=True, long_labels=True)
...
graphs.pie_chart(best_option_dict.values(), best_option_dict.keys(), 'Best Option in Terms of Mean of ROUGE-1 Recall and ROUGE-2 Recall', 'best_R_recall')
