# 3d: OREO / BERT window inputs, Legal Pegasus, surface entity chaining
...
def main(args):
    ...
    training_args = Seq2SeqTrainingArguments(
        # *** Output dir
        output_dir=f"{args.output_dir}/" + "pegasus_3d",
    ...
    # *** Model name
    output_model_fn = 'trained_pegasus_3d'
    ...
    for i, case in enumerate(mlx['test']):
        ...
        outputs = model.generate(input_tokenized,
                                 ...
                                 min_length=34,  # based on data, without entity chain (34 with)
                                 max_length=1154,  # based on data, without entity chain (1154 with)
                                 early_stopping=True,
                                 # max_new_tokens=100,
                                 do_sample=False)
        summary = tokenizer.batch_decode(outputs, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]

        case_data['oreo_pegasus_output'] = summary

        # ***
        text = case['bert_windows']
        case_data['source'] = text
    ...
    # *** filename
    filename = f"{args.output_dir}/outputs_pegasus_3d.csv"
