# ADDING TOKENS TEST
# 3: OREO inputs, Legal Pegasus, surface entity chaining
...
def main(args):
    ...
    def preprocess_pegasus(cases):
        ...
        if chain:
            special_tokens_dict = {'additional_special_tokens': '[CONTENTSPLIT]', 'additional_special_tokens': '[ENTITYCHAIN]'}
            tokenizer.add_tokens(list(special_tokens_dict))

        # *** here - input col
        inputs = cases['first_1024']
        model_inputs = tokenizer(inputs, max_length=1024, truncation=True)

        # *** here - target col and length  (960 w/o chain, just set to max 1024 with cahin)
        outputs = cases["summary/short_chain_surface"]
        if chain:
            outputs = [x.replace('[SUMMARY]', '[CONTENTSPLIT]') for x in outputs]
        labels = tokenizer(text_target=outputs, max_length=1024, truncation=True)    #return_tensors='pt' needed???
    ...
    tokenizer = PegasusTokenizer.from_pretrained(MODEL_PATH)
    if chain:
        special_tokens_dict = {'additional_special_tokens': '[ENTITYCHAIN]','additional_special_tokens': '[CONTENTSPLIT]'}
        tokenizer.add_tokens(list(special_tokens_dict))
    ...
    training_args = Seq2SeqTrainingArguments(
        # *** Output dir
        output_dir=f"{args.output_dir}/" + "pegasus_3_TOKEN_TEST",
    ...
    # *** Model name
    output_model_fn = 'trained_pegasus_3_TOKEN_TEST'
    ...
    # *** Column name
    text = mlx['test'][0]['summary/short_chain_surface']
    ...
    for i, case in enumerate(mlx['test']):
        ...
        # *** target col
        gold = case['summary/short_chain_surface']
        case_data['gold'] = gold

        # *** input col
        text = case['first_1024']
        ...
        outputs = model.generate(input_tokenized,
                                 ...
                                 # max_new_tokens=100,
                                 do_sample=False)
        summary = tokenizer.batch_decode(outputs, skip_special_tokens=False, clean_up_tokenization_spaces=False)[0]

        case_data['pegasus_output_no_skip'] = summary

        summary = tokenizer.batch_decode(outputs, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]

        case_data['pegasus_output_skip_special'] = summary

        results_df_list.append(case_data)

    results_df = pandas.DataFrame(results_df_list)
    # *** filename
    filename = f"{args.output_dir}/outputs_pegasus_3_TOKEN_TEST_2.csv"
